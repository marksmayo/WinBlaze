...
- [~] PARTIAL: In-app update story. The check runs **automatically on launch** (`OnWindowLoaded`, once) and shows a Download/Later **ContentDialog** only when a newer release exists — silent when up to date or offline, so it never blocks a normal launch. Settings → "Check for updates" also does an on-demand check. Both use a shared WinRT HttpClient fetch of GitHub `releases/latest` + `winblaze_core::update` semver compare. Validated end-to-end via UIA: up-to-date launch shows no dialog (smoke passes); a version-forced build shows the dialog with a working Download button. Remaining: automatic download-verify-apply, which needs an external updater helper (a portable app can't overwrite its own running exe).
